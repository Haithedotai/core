import { zEvmAddress } from "@haithe/shared";
import { z } from "zod";
import { ERC20_ABI } from "../../../lib/evm/ERC20_ABI";
import type { McpToolRegistrationConfig } from "../../registry";
import type { EvmMcpConfig } from "..";
import { createEvmClient } from "../evmClient";

const inputSchema = z.object({
	tokenAddress: zEvmAddress().describe("ERC20 token contract address"),
	account: zEvmAddress().describe("Address to query balance for"),
	chainId: z.number().describe("Chain ID of the network"),
});

export default function (config: McpToolRegistrationConfig<EvmMcpConfig>) {
	const { server } = config;

	server.registerTool(
		"get-erc20-balance",
		{
			title: "Get ERC20 Token Balance",
			annotations: {
				readOnlyHint: true,
				destructiveHint: false,
				idempotentHint: true,
				openWorldHint: true,
				title: "Get ERC20 Token Balance",
			},
			description: "Get ERC20 token balance for an address on a specified chain.",
			inputSchema,
		},
		async ({ tokenAddress, account, chainId }) => {
			const client = createEvmClient({ chainId });

			const balance = await client.readContract({
				address: tokenAddress,
				abi: ERC20_ABI,
				functionName: "balanceOf",
				args: [account],
				authorizationList: [],
			});

			return {
				content: [
					{
						type: "text",
						text: `unformatted (not adjusted for token decimals) ERC20 balance of ${account} for token ${tokenAddress} on chain ${chainId}: ${balance}`,
					},
				],
			};
		},
	);
}
