...
class HaitheClient {
  ...
  constructor(options: { walletClient: viem.WalletClient; baseUrl: string, debug?: boolean }) {
  ...
  private fetch<T>(uri: string, args?: Omit<Parameters<typeof fetch>[1], "headers">): Promise<T> {
    if (!this.baseUrl) {
  ...
  updateOrganization(id: number, name: string): Promise<{
    id: number;
    name: string;
    owner: string;
    created_at: string;
  ...
  getOrganizationMembers(orgId: number): Promise<Array<{
    org_id: number;
  ...
  addOrganizationMember(orgId: number, walletAddress: string, role: 'admin' | 'member'): Promise<{
    org_id: number;
    wallet_address: string;
    role: string;
    created_at: string;
  }> {
    return this.fetch(`/v1/orgs/${orgId}/members?wallet_address=${encodeURIComponent(walletAddress)}&role=${role}`, {
      method: 'POST'
    });
  }

  updateOrganizationMemberRole(orgId: number, walletAddress: string, role: 'admin' | 'member'): Promise<{
    org_id: number;
    wallet_address: string;
    role: string;
    created_at: string;
  ...
  removeOrganizationMember(orgId: number, walletAddress: string): Promise<{
    org_id: number;
    wallet_address: string;
    role: string;
    created_at: string;
  }> {
    return this.fetch(`/v1/orgs/${orgId}/members?wallet_address=${encodeURIComponent(walletAddress)}`, {
      method: 'DELETE'
    });
  }
}
