...
describe("Organization Members", () => {
  ...
  test("get organization members - empty list", async () => {
    const members = await client.getOrganizationMembers(orgId);
    expect(Array.isArray(members)).toBe(true);
    expect(members.length).toBe(0);
  });

  test("add organization member", async () => {
  ...
  test("get organization members - with members", async () => {
    const walletAddress = walletClient2.account.address;
    ...
    expect(members[0].org_id).toBe(orgId);
    expect(members[0].wallet_address).toBe(walletAddress.toLowerCase());
    expect(members[0].role).toBe("member");
  });

  test("update organization member role", async () => {
    const walletAddress = walletClient2.account.address;
  ...
  test("remove organization member", async () => {
    const walletAddress = walletClient2.account.address;
    ...
    expect(typeof removedMember.created_at).toBe("string");
