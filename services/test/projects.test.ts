...
describe("Projects via HaitheClient", () => {
    ...
    describe("Project CRUD Operations", () => {
        ...
        test("should update project", async () => {
            ...
            const updatedProject = await client.updateProject(createdProject.id, newName);
    ...
    describe("Project Members Management", () => {
        ...
        test("should add project member", async () => {
            ...
            const member = await client.addProjectMember(projectId, walletAddress, role);
    ...
    describe("Project Permission Management", () => {
        ...
        beforeEach(async () => {
            ...
            await client.addProjectMember(projectId, walletClient2.account.address, "admin");
        });

        test("project admin can add members", async () => {
            const thirdWalletAddress = walletClient3.account.address;

            const member = await client2.addProjectMember(projectId, thirdWalletAddress, "developer");
        ...
        test("project admin can update member roles", async () => {
            ...
            const updatedMember = await client2.updateProjectMemberRole(projectId, thirdWalletAddress, "developer");
        ...
        test("project admin can remove members", async () => {
            ...
            await client2.addProjectMember(projectId, thirdWalletAddress, "developer");
            const removedMember = await client2.removeProjectMember(projectId, thirdWalletAddress);
            ...
            expect(members[0].wallet_address).toBe(walletClient2.account.address.toLowerCase());
        });
    });
    ...
    describe("Role Validation", () => {
        ...
        test("should accept valid project roles", async () => {
            ...
            const adminMember = members.find(m => m.wallet_address === walletAddress3.toLowerCase());
            expect(adminMember?.role).toBe("admin");

            // Test developer role  
            await client2.updateProjectMemberRole(projectId, walletAddress3, "developer");
            members = await client.getProjectMembers(projectId);
            const devMember = members.find(m => m.wallet_address === walletAddress3.toLowerCase());
            expect(devMember?.role).toBe("developer");

            // Test viewer role
            await client2.updateProjectMemberRole(projectId, walletAddress3, "viewer");
            members = await client.getProjectMembers(projectId);
            const viewerMember = members.find(m => m.wallet_address === walletAddress3.toLowerCase());
